        ...
        switch ($fatal) {
            ...
            "missingRequiredFields" {
                ...
                foreach ($field in $required) {
                    if (-not ($entry | Get-Member -MemberType NoteProperty | Select-Object -ExpandProperty Name) -contains $field) {
                        $missingFields += $field
                    }
                }
            ...
            switch ($repair) {
                ...
                "missingDefaults" {
                    ...
                    foreach ($key in $defaults.PSObject.Properties.Name) {
                        if (-not ($entry | Get-Member -MemberType NoteProperty | Select-Object -ExpandProperty Name) -contains $key) {
